    ...
    <div className="bg-[#F4F6F9] min-h-screen pt-28 pb-20 px-4 sm:px-6">
      <div className="max-w-7xl mx-auto space-y-12">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-2">
          ...
          <h1 className="font-serif text-3xl sm:text-4xl md:text-5xl text-[#0B132B] font-bold">Insights & Articles</h1>
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="bg-white border border-[#0B132B]/10 rounded-xl overflow-hidden animate-pulse">
                <div className="h-48 bg-[#0B132B]/8" />
                <div className="p-5 space-y-3">
                  <div className="h-3 bg-[#0B132B]/8 rounded w-1/3" />
                  <div className="h-5 bg-[#0B132B]/10 rounded w-4/5" />
                  <div className="h-4 bg-[#0B132B]/6 rounded w-full" />
                  <div className="h-4 bg-[#0B132B]/6 rounded w-3/4" />
                  <div className="h-8 bg-[#0B132B]/8 rounded w-1/3 mt-4" />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {posts.map((post) => (
              <Card key={post.id} className="overflow-hidden flex flex-col bg-white border border-[#0B132B]/10 shadow-sm hover:shadow-md transition-all rounded-xl group h-full">
                {/* Cover Image */}
                <div className="relative h-48 overflow-hidden bg-[#0B132B]/5">
                  {post.coverImage ? (
                    <Image
                      src={post.coverImage}
                      alt={post.title}
                    ...
                    <div className="w-full h-full flex items-center justify-center text-[#0B132B]/20 bg-[#0B132B]/5 text-xs font-semibold">
                ...
                <CardHeader className="p-5 pb-2">
                  <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[10px] text-[#0B132B]/40 font-bold uppercase tracking-wider">
                  ...
                  <h3 className="font-serif text-xl text-[#0B132B] font-bold line-clamp-2 mt-2 group-hover:text-[#D4AF37] transition-colors leading-snug">
                    {post.title}
                  </h3>
                </CardHeader>

                <CardContent className="px-5 py-2 flex-grow">
                  <p className="text-[#0B132B]/60 text-xs sm:text-sm font-light leading-relaxed line-clamp-3">
                    {post.content.replace(/[#*`_\[\]-]/g, "")}
                  </p>
                </CardContent>

                {/* Card Footer */}
                <CardFooter className="px-5 pb-5 pt-4 flex justify-between items-center border-t border-[#0B132B]/5 bg-[#F8F9FA]/50 mt-auto">
                  <div className="flex items-center gap-2 text-xs text-[#0B132B]/60">
                  ...
                  <Link href={`/blog/${post.slug}`}>
                    <Button variant="ghost" className="text-[#0B132B] hover:text-[#D4AF37] hover:bg-transparent p-0 text-xs font-semibold uppercase tracking-wider group transition-colors gap-1.5">
                      Read Article
